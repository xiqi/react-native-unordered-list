...
export default ({ children, unorderedListLevel, bulletUnicode, color }) => {
    ...
    return (
        <View style={styles.bulletPointLine}>
            <Text style={{color}}>{bulletUnicode ? String.fromCharCode(bulletUnicode) : (level > bullets.length ? bullets[bullets.length - 1] : bullets[level])}</Text>
